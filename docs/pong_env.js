class PongEnvJs {
    constructor(canvas) {
        ...
        this.paddle2_y = 0.0; // Opponent human (right)

        // --- Physics/Game Parameters ---
        this.paddle_speed = 12.0;
        this.ball_speed_initial = 10.0;
        this.ball_speed_increase = 0.4;
        this.current_ball_speed = this.ball_speed_initial;
        this.max_ball_speed = 24.0;
        this.opponent_speed_limit = 12.0; // Speed limit for opponent
    ...
    _internal_reset() {
        ...
        this.current_ball_speed = this.ball_speed_initial;
        this.ball_vx = direction * this.current_ball_speed * Math.cos(angle);
        this.ball_vy = this.current_ball_speed * Math.sin(angle);
        this.paddle1_y = 0.0;
        this.paddle2_y = 0.0;
    }
    ...
    _normalize_state() {
        const state = [
            ...
            this.ball_vx / this.max_ball_speed,
            this.ball_vy / this.max_ball_speed,
            this.paddle1_y / this.norm_height,
        ];
         if (state.length !== this.state_dim) {
            console.error(`State dimension mismatch in _normalize_state! Expected ${this.state_dim}, got ${state.length}`);
        }
    ...
    // opponent_paddle_y is calculated in app.js
    step(ai_action, opponent_paddle_y, deltaTime = 1/60) {

        const dt_paddle_speed = this.paddle_speed * 60 * deltaTime;
        const dt_ball_vx = this.ball_vx * 60 * deltaTime;
        const dt_ball_vy = this.ball_vy * 60 * deltaTime;

        const prev_ball_x = this.ball_x; // Store previous x for collision check

        // 1. Move AI Agent Paddle (Paddle 1)
        if (ai_action === 1) { this.paddle1_y -= dt_paddle_speed; }
        else if (ai_action === 2) { this.paddle1_y += dt_paddle_speed; }
        ...
        // 2. Update Opponent Paddle (Paddle 2) position from app.js calculation
        this.paddle2_y = opponent_paddle_y;
        ...
        if (this.ball_vx < 0 && prev_ball_x - this.ball_radius >= paddle1_front_edge && this.ball_x - this.ball_radius < paddle1_front_edge && Math.abs(this.ball_y - this.paddle1_y) < paddle_half_height + this.ball_radius) {
            ...
            this.current_ball_speed = Math.min(this.current_ball_speed + this.ball_speed_increase, this.max_ball_speed);
            this.ball_vx = this.current_ball_speed * Math.cos(bounce_angle);
            this.ball_vy = this.current_ball_speed * -Math.sin(bounce_angle);
            this.ball_x = paddle1_front_edge + this.ball_radius; // Correct position
            paddle_hit = true;
        }
        // Collision with Opponent Paddle (Paddle 2)
            ...
            this.ball_vx = -this.current_ball_speed * Math.cos(bounce_angle);
            this.ball_vy = this.current_ball_speed * -Math.sin(bounce_angle);
            this.ball_x = paddle2_front_edge - this.ball_radius; // Correct position
            paddle_hit = true;
        }

        //   c. Scoring
        if (!paddle_hit) {
            if (this.ball_x - this.ball_radius < -this.norm_width) { reward = -1.0; terminated = true; } // Opponent scores
            else if (this.ball_x + this.ball_radius > this.norm_width) { reward = 1.0; terminated = true; } // Agent scores
        }

        // 5. Prepare return values
        const observation = this._normalize_state(); // Get 5D state
